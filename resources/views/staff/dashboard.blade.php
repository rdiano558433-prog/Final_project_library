...
<div class="py-4">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr>
                        ...
                        {{-- SAFE USER --}}
                        <td class="py-2">
                            {{ \Illuminate\Support\Str::limit($b->user?->name ?? 'Unknown User', 15) }}
                        </td>
    ...
    {{ Str::limit($b->book?->title ?? 'Deleted Book', 20) }}
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-6 text-center text-gray-400">No records.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-5">
            ...
            <div class="space-y-2">
                <a href="{{ route('staff.borrowings.create') }}"
                   class="flex items-center gap-3 w-full bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-3 text-sm font-medium">
                    📤 Issue a Book
                </a>

                <a href="{{ route('staff.books.create') }}"
                   class="flex items-center gap-3 w-full bg-purple-600 hover:bg-purple-700 text-white rounded-lg px-4 py-3 text-sm font-medium">
                    📚 Add Book
                </a>

                <a href="{{ route('staff.reports.overdue') }}"
                   class="flex items-center gap-3 w-full bg-red-600 hover:bg-red-700 text-white rounded-lg px-4 py-3 text-sm font-medium">
                    ⚠️ View Overdue
                </a>
            </div>
        </div>
