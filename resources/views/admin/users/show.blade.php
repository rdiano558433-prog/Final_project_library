...
<div class="py-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            @if($book->category)
                <span class="inline-block bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded mb-3">
                    {{ $book->category }}
                </span>
            @endif
            ...
            <div class="space-y-2 text-sm mt-4">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold">{{ $book->total_copies }}</span>
                </div>
            ...
            {{-- Admin/Staff Actions --}}
            @if(auth()->check() && method_exists(auth()->user(), 'isAdminOrStaff') && auth()->user()->isAdminOrStaff())

                @php
                    $role = auth()->user()->role;

                    $editRoute = $role === 'admin'
                        ? route('admin.books.edit', $book)
                        : route('staff.books.edit', $book);

                    $indexRoute = $role === 'admin'
                        ? route('admin.books.index')
                        : route('staff.books.index');

                    $deleteRoute = $role === 'admin'
                        ...
                        @forelse($borrowings ?? [] as $b)
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <span class="px-2 py-0.5 rounded-full text-xs {{ $sc[$b->status] ?? '' }}">
                                        {{ ucfirst($b->status) }}
                                    </span>
                                </td>
                            </tr>
